...
const ShortParagraph = () => {
  ...
  const fetchParagraph = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/lessons/short-paragraphs/one?module=${module}&level=${level}&sequence=${sequence}`,
        {
      ...
      toast({
        title: 'Error',
        description: 'Failed to load paragraph',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async () => {
    if (!answer.trim()) {
      toast({
        title: 'Error',
        description: 'Please write your paragraph',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(
        `${API_BASE_URL}/composition/shortParagraphs/submit`,
        {
          method: 'POST',
          headers: {
      ...
      <div className="min-h-screen bg-gradient-soft flex items-center justify-center">
        <Card className="max-w-md">
          <CardContent className="pt-6 text-center">
            <p className="text-muted-foreground">Paragraph not found</p>
            <Button
              onClick={() => navigate('/short-paragraphs')}
              className="mt-4"
            >
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <Button
              variant="ghost"
              onClick={() =>
                navigate(`/short-paragraphs?module=${module}&level=${level}`)
              }
            >
              <ArrowLeft className="w-4 h-4 mr-2" />
              Back
        ...
        <Card className="shadow-medium">
          <CardHeader>
            <CardTitle className="text-2xl">
              Short Paragraph #{paragraph.sequence}
            </CardTitle>
              ...
              <Card className="bg-gradient-soft border-0">
                ...
                <CardContent className="space-y-4">
                  ...
                  <Button
                    onClick={() =>
                      navigate(
                        `/short-paragraphs?module=${module}&level=${level}`
                      )
                    }
                    className="w-full"
                  >
